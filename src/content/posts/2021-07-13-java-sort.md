...
date: '2021-07-13T19:09:00+09:00'
permalink: /2021/07/13/java-sort/
section: backend
hub: java
type: reference
level: 중급
...
```java
// 1) 단순 String 정렬
List<String> list = Arrays.asList("betty", "alice", "robert");
Collections.sort(list); // [alice, betty, robert] 오름차순으로 나온다.
...
List<String> result = list.stream().sorted(String::compareTo).collect(Collectors.toList()); //[alice, betty, robert]
```
...
![스크린샷 2021-07-13 오후 6 59 18](/images/posts/43f4b21811a8.png)
...
// 나이순 오름차순으로 정렬됨
//[User(name=betty, age=24), User(name=andy, age=24), User(name=alice, age=32), User(name=robert, age=40)]
```
...
//나이, 이름순으로 정렬
//[User(name=andy, age=24), User(name=betty, age=24), User(name=alice, age=32), User(name=robert, age=40)]
```
