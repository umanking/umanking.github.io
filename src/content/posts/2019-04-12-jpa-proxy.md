...
description: >-
  프록시의 기본과, JPA에서 fetch 전략이 어떤 것들이 있는 지 알아보자. PA에서 식별자로 엔티티 하나를 조회할 때
  EntityManager.find() 를 사용한다. 이 메서드는 영속성 컨텍스트에 엔티티가 없으면 데이터 베이스를 조회한다.
date: '2019-04-12T00:00:00+09:00'
permalink: /2019/04/12/jpa-proxy/
section: backend
hub: jpa
type: reference
level: 중급
...
```java
    ...
    public void lazy로딩_teamTest() {
        ...
        //Then
        Assert.assertEquals(existTeam.getMemberList().get(0).getName(), "alice");
